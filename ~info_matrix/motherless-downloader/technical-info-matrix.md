# Technical Info Matrix — Motherless Downloader

## Extension: `motherless-downloader`

### Core Identity

| Field | Value |
|---|---|
| **Extension Name** | SERP Motherless Downloader |
| **Slug / ID** | `motherless-downloader` |
| **Gecko ID** | `motherless-downloader@serpapps.com` |
| **Category** | Video Downloader (Adult) |
| **Target Site(s)** | motherless.com and subdomains |
| **Description** | Download Motherless videos |
...
| **Manifest Spec** | MV3 |
| **Branch Name** | `legacy/motherless-downloader` |
...
| **Store-Sanitized Build?** | Yes (`motherless-downloader-chrome-store-sanitized.zip`) |
| **GitHub Releases Repo** | `serpapps/motherless-downloader` |
| **Product Page** | https://serp.ly/motherless-downloader |
...
| **Context Menu** | Yes — "Download Motherless Video" on page and video contexts |
| **Auto-Save** | Yes — saves to `Downloads/Motherless/` folder, no save-as dialog |
...
| **Host Permissions** | `https://api.github.com/*`, `https://auth.serp.co/*`, `https://motherless.com/*`, `https://*.motherless.com/*` |
| **Content Scripts** | `site-config.js` → `logger.js` → `download-manager.js` → `content.js` → `player-button.js` — injected on `motherless.com` at `document_idle` |
...
|---|---|
| **Site Match** | `https://motherless.com/*`, `https://*.motherless.com/*` |
| **Video ID Patterns** | `/motherless\.com\/([A-Z0-9]{6,10})(?:[/?#]\|$)/i`, `[?&](?:v\|id\|video_id)=([A-Z0-9]{6,10})` |
...
| **Format Sources** | A) flashvars_* mediaDefinitions, B) HTML5 `<video>`/`<source>`, C) inject.js page-context extraction |
...
| **Panel ID** | `motherless-download-manager` |
| **Card Width** | 380px |
...
| Field | Value |
|---|---|
| **Menu ID** | `download-motherless-video` |
| **Title** | "Download Motherless Video" |
| **Contexts** | `["page", "video"]` |
| **URL Patterns** | `motherless.com/*` and subdomains |

### Popup UI

| Property | Value |
|---|---|
| **Stylesheets** | `styles/styles.css` (8.8 KB), `styles/popup-enhanced.css` (16.4 KB) |
...
| **Sections** | Header, Activation section, Loading spinner, Error state, Video info card, Quality selector, Download button |
...
| **Watermarked?** | Yes |
| **GitHub Release?** | Yes — `serpapps/motherless-downloader` |
| **Has Worktree?** | Yes — `.worktrees/motherless-downloader/` |

#### Platform Builds

| Platform | File | Size |
|---|---|---|
| Standard | `motherless-downloader.zip` | <!-- TODO --> |
| Chrome | `motherless-downloader-chrome.zip` | <!-- TODO --> |
| Chrome Store Sanitized | `motherless-downloader-chrome-store-sanitized.zip` | <!-- TODO --> |
| Brave | `motherless-downloader-brave.zip` | <!-- TODO --> |
| Edge | `motherless-downloader-edge.zip` | <!-- TODO --> |
| Opera | `motherless-downloader-opera.zip` | <!-- TODO --> |
| Whale | `motherless-downloader-whale.zip` | <!-- TODO --> |
| Yandex | `motherless-downloader-yandex.zip` | <!-- TODO --> |
| Firefox ZIP | `motherless-downloader-firefox.zip` | <!-- TODO --> |
| Firefox XPI | `motherless-downloader-firefox-unpacked.xpi` | <!-- TODO --> |
...
| **Stripe Product ID** | `prod_TfxYCCYzli4aiG` |
| **Stripe Product Name** | Motherless Downloader |
| **Stripe Monthly Price** | USD 9.00/month [motherless-downloader monthly] |
| **Stripe One-Time Price** | USD 17.00/one_time |
| **Stripe Price IDs** | `price_1SibdcDP7AOTRcvmS6viCC9e`, `price_1SymszDP7AOTRcvmCD8vUx9N` |
