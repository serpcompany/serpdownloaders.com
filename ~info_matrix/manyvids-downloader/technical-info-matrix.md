# Technical Info Matrix — ManyVids Downloader
...
| Field | Value |
|---|---|
| **Extension Name** | SERP ManyVids Downloader |
| **Slug / ID** | `manyvids-downloader` |
| **Gecko ID** | `manyvids-downloader@serpapps.com` |
| **Category** | Video Downloader (Adult) |
| **Target Site(s)** | manyvids.com and subdomains |
| **Description** | Download ManyVids videos |
...
| **Manifest Spec** | MV3 |
| **Branch Name** | `legacy/manyvids-downloader` |
...
| **Store-Sanitized Build?** | Yes (`manyvids-downloader-chrome-store-sanitized.zip`) |
| **GitHub Releases Repo** | `serpapps/manyvids-downloader` |
| **Product Page** | https://serp.ly/manyvids-downloader |
...
| **Context Menu** | Yes — "Download ManyVids Video" on page and video contexts |
| **Auto-Save** | Yes — saves to `Downloads/ManyVids/` folder, no save-as dialog |
...
| **Host Permissions** | `https://api.github.com/*`, `https://auth.serp.co/*`, `https://manyvids.com/*`, `https://*.manyvids.com/*` |
| **Content Scripts** | `site-config.js` → `logger.js` → `download-manager.js` → `content.js` → `player-button.js` — injected on `manyvids.com` at `document_idle` |
...
|---|---|
| **Site Match** | `https://manyvids.com/*`, `https://*.manyvids.com/*` |
| **Video ID Patterns** | `/video/(\d+)(?:[/?#-]\|$)`, `[?&](?:video_id\|id)=(\d+)` |
...
| **Format Sources** | A) flashvars_* mediaDefinitions, B) HTML5 `<video>`/`<source>`, C) Script regex for mp4/m3u8 URLs, D) inject.js XHR/fetch intercept |
...
| **Panel ID** | `manyvids-download-manager` |
| **Card Width** | 380px |
...
| Field | Value |
|---|---|
| **Menu ID** | `download-manyvids-video` |
| **Title** | "Download ManyVids Video" |
| **Contexts** | `["page", "video"]` |
| **URL Patterns** | `manyvids.com/*` and subdomains |
...
| **Watermarked?** | Yes |
| **GitHub Release?** | Yes — `serpapps/manyvids-downloader` |
| **Has Worktree?** | Yes — `.worktrees/manyvids-downloader/` |

#### Platform Builds

| Platform | File | Size |
|---|---|---|
| Standard | `manyvids-downloader.zip` | <!-- TODO --> |
| Chrome | `manyvids-downloader-chrome.zip` | <!-- TODO --> |
| Chrome Store Sanitized | `manyvids-downloader-chrome-store-sanitized.zip` | <!-- TODO --> |
| Brave | `manyvids-downloader-brave.zip` | <!-- TODO --> |
| Edge | `manyvids-downloader-edge.zip` | <!-- TODO --> |
| Opera | `manyvids-downloader-opera.zip` | <!-- TODO --> |
| Whale | `manyvids-downloader-whale.zip` | <!-- TODO --> |
| Yandex | `manyvids-downloader-yandex.zip` | <!-- TODO --> |
| Firefox ZIP | `manyvids-downloader-firefox.zip` | <!-- TODO --> |
| Firefox XPI | `manyvids-downloader-firefox-unpacked.xpi` | <!-- TODO --> |
...
| **Stripe Product ID** | `prod_TfxYf5SkjvCwgJ` |
| **Stripe Product Name** | ManyVids Downloader |
| **Stripe Monthly Price** | USD 9.00/month [manyvids-downloader monthly] |
| **Stripe One-Time Price** | USD 17.00/one_time |
| **Stripe Price IDs** | `price_1SibdbDP7AOTRcvm8KrWQfLg`, `price_1SymsyDP7AOTRcvmsn0UFdzC` |
