# Technical Info Matrix — nhentai Downloader
...
| Field | Value |
|---|---|
| **Extension Name** | Video Downloader for NHentai |
| **Slug / ID** | `nhentai-downloader` |
| **Gecko ID** | `nhentai-downloader@serpapps.com` |
| **Category** | Video/Media Downloader (Adult) |
| **Target Site(s)** | nhentai.net and subdomains |
| **Description** | Download NHentai videos |
...
| **Manifest Spec** | MV3 |
| **Branch Name** | `legacy/nhentai-downloader` |
...
| **Store-Sanitized Build?** | Yes (`nhentai-downloader-chrome-store-sanitized.zip`) |
| **GitHub Releases Repo** | `serpapps/nhentai-downloader` |
| **Product Page** | https://serp.ly/nhentai-downloader |
...
| **Context Menu** | Yes — "Download NHentai Video" on page and video contexts |
| **Auto-Save** | Yes — saves to `Downloads/NHentai/` folder, no save-as dialog |
| **Desktop Notifications** | Yes — "NHentai Download Complete" |
...
| **Host Permissions** | `https://api.github.com/*`, `https://auth.serp.co/*`, `https://nhentai.net/*`, `https://*.nhentai.net/*` |
| **Content Scripts** | `site-config.js` → `logger.js` → `download-manager.js` → `content.js` → `player-button.js` — injected on `nhentai.net` at `document_idle` |
...
| Bootstrap | `bootstrap.js` | Extension initialization |

### Video Detection & Extraction

| Feature | Implementation |
|---|---|
| **Site Match** | `https://nhentai.net/*`, `https://*.nhentai.net/*` |
...
| **Format Sources** | A) flashvars_* mediaDefinitions, B) HTML5 `<video>`/`<source>`, C) Script regex for mp4/m3u8 URLs, D) Media API response arrays |
| **CDN Detection** | Performance entries matching nhentai.net CDN patterns |
| **Media Request Patterns** | `mediaDefinitions`, `.m3u8`, `.mp4`, HLS format detection (`format === 'hls'`, `.m3u8` in URL) |
...
| **Panel ID** | `nhentai-download-manager` |
| **Card Width** | 380px |
...
| Field | Value |
|---|---|
| **Menu ID** | `download-nhentai-video` |
| **Title** | "Download NHentai Video" |
| **Contexts** | `["page", "video"]` |
| **URL Patterns** | `nhentai.net/*` and subdomains |
...
| **Script Load Order** | `auth.js` (module) → `popup.js` (module) → `styles/styles.css` → `styles/popup-enhanced.css` → `auth-ui.js` (defer) → `trial-banner.js` (defer) → `site-config.js` → `update-notifier.js` (defer) |
...
| Field | Value |
|---|---|
| **Build Tool** | `build.js` (local build script) |
| **Watermarked?** | Yes |
| **GitHub Release?** | Yes — `serpapps/nhentai-downloader` |
| **Has Worktree?** | Yes — `.worktrees/nhentai-downloader/` |

#### Platform Builds

| Platform | File | Size |
|---|---|---|
| Standard | `nhentai-downloader.zip` | <!-- TODO --> |
| Chrome | `nhentai-downloader-chrome.zip` | <!-- TODO --> |
| Chrome Store Sanitized | `nhentai-downloader-chrome-store-sanitized.zip` | <!-- TODO --> |
| Brave | `nhentai-downloader-brave.zip` | <!-- TODO --> |
| Edge | `nhentai-downloader-edge.zip` | <!-- TODO --> |
| Opera | `nhentai-downloader-opera.zip` | <!-- TODO --> |
| Whale | `nhentai-downloader-whale.zip` | <!-- TODO --> |
| Yandex | `nhentai-downloader-yandex.zip` | <!-- TODO --> |
| Firefox ZIP | `nhentai-downloader-firefox.zip` | <!-- TODO --> |
| Firefox XPI | `nhentai-downloader-firefox-unpacked.xpi` | <!-- TODO --> |
...
| **Stripe Product ID** | `prod_TfxYd9hCfO3b9j` |
| **Stripe Product Name** | NHentai Downloader |
| **Stripe Monthly Price** | USD 9.00/month [nhentai-downloader monthly] |
| **Stripe One-Time Price** | USD 17.00/one_time |
| **Stripe Price IDs** | `price_1SibddDP7AOTRcvm5Cog7M4L`, `price_1SymszDP7AOTRcvmL5HUSZwk` |
...
| YouTube | `modules/youtube/` | YouTube client, signature, SABR, UMP, media assembler |
...
|---|---|---|
| `archiver` | ^7.0.1 | ZIP archive creation for builds |
| `sharp` | ^0.34.4 | Image processing (icon generation) |
