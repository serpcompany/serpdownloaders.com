# Technical Info Matrix — Cam4 Downloader

## Extension: `cam4-downloader`

### Core Identity

| Field | Value |
|---|---|
| **Extension Name** | SERP Cam4 Downloader |
| **Slug / ID** | `cam4-downloader` |
| **Gecko ID** | `cam4-downloader@serpapps.com` |
| **Category** | Video Downloader (Adult / Live Cam) |
| **Target Site(s)** | cam4.com and subdomains |
| **Description** | Download Cam4 live streams and recordings |
...
| **Manifest Spec** | MV3 |
| **Branch Name** | `legacy/cam4-downloader` |
...
| **GitHub Releases Repo** | `serpapps/cam4-video-downloader` |
| **Product Page** | https://serp.ly/cam4-video-downloader |
...
| **Live Stream Support?** | Yes — continuous LL-HLS segment recording with Start/Stop controls |
| **VOD Support?** | Yes — `/{model}/videos/{videoId}` pattern |
| **VR Support?** | Yes — VR camera settings (stereoPacking, frameFormat, horizontalAngle); VR filename suffix appended |
| **Bulk Download?** | No |
...
| **Context Menu** | Yes — "Download Cam4 Stream" on page and video contexts |
| **Auto-Save** | Yes — saves to `Downloads/CAM4/` folder, no save-as dialog |
| **Desktop Notifications** | No (permission declared but not used) |
| **Video Detection** | Cam4 REST API profile/info + streamInfo endpoints via inject.js, HLS master manifest probing across multiple CDN hosts |
...
| **Host Permissions** | `https://api.github.com/*`, `https://auth.serp.co/*`, `https://*/*`, `https://cam4.com/*`, `https://www.cam4.com/*`, `https://hu.cam4.com/*`, `https://webchat.cam4.com/*`, `https://*.cam4.com/*` |
| **Content Scripts** | `site-config.js` -> `logger.js` -> `download-manager.js` -> `content.js` -> `player-button.js` — injected on `cam4.com` at `document_idle` |
...
| **Page Injection** | `inject.js` — Cam4 REST API calls (profile info, streamInfo, requestAccess), HLS URL collection, posts `CAM4_PAGE_DATA` messages |
| **External APIs Called** | SERP Auth (`auth.serp.co`), GitHub Releases API (`api.github.com`), Cam4 Profile Info, Cam4 Stream Info, Cam4 Access API, Cam4 Front API, Cam4 Videos API |
...
| Service Worker | `background-enhanced.js` (module) | Download orchestration, HLS master manifest parsing, Cam4 API integration, Mouflon decryption, auth gating, offscreen management, context menu |
...
| Inject | `inject.js` | Page-context Cam4 REST API calls, HLS URL collection, model metadata |
...
|---|---|
| **Site Match** | `https://cam4.com/*`, `https://www.cam4.com/*`, `https://hu.cam4.com/*`, `https://*.cam4.com/*` |
| **Page Type Detection** | URL path matching: `/{model}/videos/{id}` = VOD, all other model pages = live |
| **Model ID Sources** | Cam4 profile info API (id, userId), inject.js roomSlug from URL path, API streamName, API userId |
| **Title Sources** | `info.stageName`, `info.username`, model displayName, `meta[property="og:title"]`, `document.title` |
| **Thumbnail Sources** | `meta[property="og:image"]`, `video.currentSrc` |
| **Stream Host Sources** | `cam.hlsStreamHost`, `cam.streamHost`, `cam.stream_server.hlsStreamHost`, API streamInfo cdnURL, config fallback domains |
...
| **Inject Message Type** | `CAM4_PAGE_DATA` / `REQUEST_CAM4_DATA` |
| **Cam4 API Endpoints** | `hu.cam4.com/rest/v1.0/profile/{username}/info`, `hu.cam4.com/rest/v1.0/profile/{username}/streamInfo`, `webchat.cam4.com/requestAccess?roomname={username}` |

### Mouflon DRM

| Field | Value |
|---|---|
| **Description** | Cam4 uses the Mouflon HLS manifest obfuscation system (shared with Stripchat) that encrypts segment filenames |
...
| **Manifest Rewrite** | Decoded filenames replace Forbidden or media.mp4 placeholders |
...
| **Playlist Parse Timeout** | 12,000ms |
| **LL-HLS Support** | Yes (EXT-X-PART, _HLS_msn/_HLS_part hints) |
...
| **Credential Mode** | cam4/CDN domains = include, others = omit |
| **Fetch Fallback** | Background fetch -> offscreen fetch (page-like headers/UA) |
| **Referer** | `https://cam4.com/` |
| **Origin** | `https://cam4.com` |
...
| **Panel ID** | `CAM4-download-manager` |
| **Card Width** | 380px |
...
| Field | Value |
|---|---|
| **Menu ID** | `download-cam4-video` |
| **Title** | "Download Cam4 Stream" |
| **Contexts** | `["page", "video"]` |
| **URL Patterns** | `cam4.com/*`, `*.cam4.com/*`, `m.cam4.com/*` |
...
| **Sections** | Quick help banner, Header, Activation section, Loading spinner, Error state, Video info card (thumbnail, title, duration, VR badge), Quality selector, Capture hint, Live capture status, Download button (Start/Stop for live), Progress bar |
...
| **Watermarked?** | Yes |
| **GitHub Release?** | Yes — `serpapps/cam4-video-downloader` |
| **Has Worktree?** | Yes — `.worktrees/cam4-downloader/` |
...
| Cache | TTL | Purpose |
|---|---|---|
| `CAM4MetaCache` | 15,000ms | Stream metadata per model slug |
| `CAM4VideosCache` | 15,000ms | Videos list per user ID |
| `CAM4ConfigCache` | 300,000ms (5 min) | Cam4 config (Mouflon origins) |
...
| **Stripe Product ID** | `prod_U5nhsrNyp5djNw` |
| **Stripe Product Name** | Cam4 Downloader |
| **Stripe Monthly Price** | USD 9.00/month [cam4-downloader-monthly-9] |
| **Stripe One-Time Price** | (none) |
| **Stripe Price IDs** | `price_1T7c65DP7AOTRcvmbXKPDyyS` |
