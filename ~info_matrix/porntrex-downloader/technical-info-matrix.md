# Technical Info Matrix — PornTrex Downloader
...
| Field | Value |
|---|---|
| **Extension Name** | Video Downloader for PornTrex |
| **Slug / ID** | `porntrex-downloader` |
| **Gecko ID** | `porntrex-downloader@serpapps.com` |
| **Category** | Video Downloader (Adult) |
| **Target Site(s)** | porntrex.com and subdomains |
| **Description** | Download PornTrex videos |
...
| **Manifest Spec** | MV3 |
| **Branch Name** | `legacy/porntrex-downloader` |
...
| **Store-Sanitized Build?** | Yes (`porntrex-downloader-chrome-store-sanitized.zip`) |
| **GitHub Releases Repo** | `serpapps/porntrex-downloader` |
| **Product Page** | https://serp.ly/porntrex-downloader |
...
| **Context Menu** | Yes — "Download PornTrex Video" on page and video contexts |
| **Auto-Save** | Yes — saves to `Downloads/PornTrex/` folder, no save-as dialog |
...
| **Host Permissions** | `https://api.github.com/*`, `https://auth.serp.co/*`, `https://porntrex.com/*`, `https://*.porntrex.com/*` |
| **Content Scripts** | `site-config.js` → `logger.js` → `download-manager.js` → `content.js` → `player-button.js` — injected on `porntrex.com` at `document_idle` |
...
|---|---|
| **Site Match** | `https://porntrex.com/*`, `https://*.porntrex.com/*` |
| **Video ID Patterns** | `/video/(\d+)(?:/\|$)`, `/videos/(\d+)(?:/\|$)`, `/watch/(\d+)(?:/\|$)`, `[?&](?:video_id\|videoId\|id\|v)=([a-zA-Z0-9]+)`, `viewkey=([a-zA-Z0-9]+)` |
| **Title Sources** | `h1.title`, `h1`, `[data-video-title]`, `meta[property="og:title"]` |
...
| **Extra Metadata** | Views (`.count`, `[class*="views"]`), Likes (`.votesUp`), Dislikes (`.votesDown`), Tags (`.tagsWrapper a`), Categories (`.categoriesWrapper a`), Uploader (`.usernameBadgesWrapper a`, `.video-detailed-info a.bolded`, `.username`, `a[href*="/members/"]`) |
| **Format Sources** | A) flashvars_* mediaDefinitions, B) HTML5 `<video>`/`<source>`, C) Script regex for mp4/m3u8 URLs, D) Performance API CDN entries, E) inject.js XHR/fetch monitor |
| **Script URL Regex** | `/https?:\\?\\/\\?\\/[^"'\\\s]+?\\.(?:m3u8\|mp4)(?:\\?[^"'\\\s]*)?/gi` |
| **CDN Detection** | Performance entries matching `/cdntrex\.com/i` and `/sadbaguette\.com/i` |
...
| **Panel ID** | `porntrex-download-manager` |
| **Card Width** | 380px |
...
| Field | Value |
|---|---|
| **Menu ID** | `download-porntrex-video` |
| **Title** | "Download PornTrex Video" |
| **Contexts** | `["page", "video"]` |
| **URL Patterns** | `porntrex.com/*` and subdomains |

### Popup UI

| Property | Value |
|---|---|
| **Stylesheets** | `styles/styles.css` (8.8 KB), `styles/popup-enhanced.css` (14.6 KB) |
...
| **Watermarked?** | Yes |
| **GitHub Release?** | Yes — `serpapps/porntrex-downloader` |
| **Has Worktree?** | Yes — `.worktrees/porntrex-downloader/` |

#### Platform Builds

| Platform | File | Size |
|---|---|---|
| Standard | `porntrex-downloader.zip` | <!-- TODO --> |
| Chrome | `porntrex-downloader-chrome.zip` | <!-- TODO --> |
| Chrome Store Sanitized | `porntrex-downloader-chrome-store-sanitized.zip` | <!-- TODO --> |
| Brave | `porntrex-downloader-brave.zip` | <!-- TODO --> |
| Edge | `porntrex-downloader-edge.zip` | <!-- TODO --> |
| Opera | `porntrex-downloader-opera.zip` | <!-- TODO --> |
| Whale | `porntrex-downloader-whale.zip` | <!-- TODO --> |
| Yandex | `porntrex-downloader-yandex.zip` | <!-- TODO --> |
| Firefox ZIP | `porntrex-downloader-firefox.zip` | <!-- TODO --> |
| Firefox XPI | `porntrex-downloader-firefox-unpacked.xpi` | <!-- TODO --> |
...
| **Stripe Product ID** | `prod_TfxYMfLyOWzUJD` |
| **Stripe Product Name** | PornTrex Downloader |
| **Stripe Monthly Price** | USD 9.00/month [porntrex-downloader monthly] |
| **Stripe One-Time Price** | USD 17.00/one_time |
| **Stripe Price IDs** | `price_1SibdeDP7AOTRcvmblCWr1TB`, `price_1Symt2DP7AOTRcvmkD6WMTGo` |
