# Technical Info Matrix — LuxureTV Downloader
...
| Field | Value |
|---|---|
| **Extension Name** | SERP LuxureTV Downloader |
| **Slug / ID** | `luxuretv-downloader` |
| **Gecko ID** | `luxuretv-downloader@serpapps.com` |
| **Category** | Video Downloader (Adult) |
| **Target Site(s)** | luxuretv.com and subdomains |
| **Description** | Download LuxureTV videos |
...
| **Manifest Spec** | MV3 |
| **Branch Name** | `legacy/luxuretv-downloader` |
...
| **Store-Sanitized Build?** | Yes (`luxuretv-downloader-chrome-store-sanitized.zip`) |
| **GitHub Releases Repo** | `serpapps/luxuretv-downloader` |
| **Product Page** | https://serp.ly/luxuretv-downloader |
...
| **Quality Selection** | Yes — parsed from flashvars mediaDefinitions, `<video>`/`<source>` tags, script regex, Video.js player sources |
...
| **Context Menu** | Yes — "Download LuxureTV Video" on page and video contexts |
| **Auto-Save** | Yes — saves to `Downloads/LuxureTV/` folder, no save-as dialog |
| **Desktop Notifications** | Yes — "Download Complete" / "Download Failed" |
| **Video Detection** | Automatic — flashvars, HTML5 video, Video.js players, script regex, inject.js XHR/fetch monitor |
...
| **Host Permissions** | `https://api.github.com/*`, `https://auth.serp.co/*`, `https://luxuretv.com/*`, `https://*.luxuretv.com/*` |
| **Content Scripts** | `site-config.js` → `logger.js` → `download-manager.js` → `content.js` → `player-button.js` — injected on `luxuretv.com` at `document_idle` |
...
|---|---|
| **Site Match** | `https://luxuretv.com/*`, `https://*.luxuretv.com/*` |
| **Video ID Patterns** | `/videos/[^/?#]*-(\d+)\.html(?:[?#]\|$)`, `/embed/(\d+)(?:[/?#]\|$)`, `[?&](?:idvideo\|video_id\|id)=(\d+)` |
| **Title Sources** | `h1`, `h1.title`, `[data-video-title]`, `meta[name="twitter:title"]`, `meta[property="og:title"]` |
| **Thumbnail Sources** | `og:image`, `<video poster>` |
| **Duration Sources** | `<video>` element, `flashvars.video_duration`, `flashvars.videoDuration`, text regex (`HH:MM:SS` / `MM:SS`) |
| **Extra Metadata** | Views (`.count`), Likes (`.votesUp`), Dislikes (`.votesDown`), Tags (`.tagsWrapper a`), Categories (`.categoriesWrapper a`, `a[href*="/channels/"]`), Uploader (`a[href*="/profiles/"] strong`, `.usernameBadgesWrapper a`, `.video-detailed-info a.bolded`, `.username`) |
| **Format Sources** | A) flashvars_* mediaDefinitions, B) HTML5 `<video>`/`<source>`, C) Video.js player sources, D) inject.js page-context data |
| **Media Request Patterns** | `/video/get_media`, `mediaDefinitions` |
...
| Parameter | Value |
|---|---|
| **Converter** | Background M3U8 parser + offscreen segment transmuxer |
| **M3U8 Parsing** | Background service worker (`extractM3U8Formats`) — master and media playlist support |
| **Segment Processing** | Offscreen document (`PROCESS_HLS_SEGMENTS` message) |
| **Referer** | `https://luxuretv.com/` (from video page URL) |
...
| **Panel ID** | `luxuretv-download-manager` |
| **Card Width** | 380px |
...
| Field | Value |
|---|---|
| **Menu ID** | `download-luxuretv-video` |
| **Title** | "Download LuxureTV Video" |
| **Contexts** | `["page", "video"]` |
| **URL Patterns** | `luxuretv.com/*` and subdomains |

### Popup UI

| Property | Value |
|---|---|
| **Stylesheets** | `styles/styles.css` (10.8 KB), `styles/popup-enhanced.css` (15.6 KB) |
...
| **Build Tool** | `build.js` |
| **Watermarked?** | Yes |
| **GitHub Release?** | Yes — `serpapps/luxuretv-downloader` |
| **Has Worktree?** | Yes — `.worktrees/luxuretv-downloader/` |

#### Platform Builds

| Platform | File | Size |
|---|---|---|
| Standard | `luxuretv-downloader.zip` | <!-- TODO --> |
| Chrome | `luxuretv-downloader-chrome.zip` | <!-- TODO --> |
| Chrome Store Sanitized | `luxuretv-downloader-chrome-store-sanitized.zip` | <!-- TODO --> |
| Brave | `luxuretv-downloader-brave.zip` | <!-- TODO --> |
| Edge | `luxuretv-downloader-edge.zip` | <!-- TODO --> |
| Opera | `luxuretv-downloader-opera.zip` | <!-- TODO --> |
| Whale | `luxuretv-downloader-whale.zip` | <!-- TODO --> |
| Yandex | `luxuretv-downloader-yandex.zip` | <!-- TODO --> |
| Firefox ZIP | `luxuretv-downloader-firefox.zip` | <!-- TODO --> |
| Firefox XPI | `luxuretv-downloader-firefox-unpacked.xpi` | <!-- TODO --> |
...
| **Stripe Product ID** | `prod_TfxY8xXqddCchY` |
| **Stripe Product Name** | LuxureTV Downloader |
| **Stripe Monthly Price** | USD 9.00/month [luxuretv-downloader monthly] |
| **Stripe One-Time Price** | USD 17.00/one_time |
| **Stripe Price IDs** | `price_1SibdaDP7AOTRcvmKs7oaxsX`, `price_1SymsxDP7AOTRcvmZZke3Wz0` |
...
| YouTube | `modules/youtube/` | YouTube media utilities (client, signature, SABR, UMP) |
